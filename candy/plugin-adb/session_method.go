...
// session_method.go — the adb: session method (plan Cutover E, E-4): the
// ON-DEVICE screenrecord bracket driven through the runner's GENERIC
// background-session service (plugin-check's compiled-in verb:session seam,
// candy/plugin-check/session_seam.go) — the plugin-vnc session pattern (E-1)
// applied to the adb transport. The provider submits a GENERIC host-step request
// — its recorder command + session identity, NEVER a systemd unit — and the
// runner dispatches it; the provider never learns the transport, spawns no
// process, owns no pidfile. The recorder (record.go) owns the device wire: it
// launches the device-side `screenrecord` at start, holds it across the phase,
// and on stop SIGINTs it, pulls the finalized MP4 via the goadb device-file
// GetFile path, and finalizes the evidence row.
//
// Wire shape (the runner's contract, session_seam.go):
//
...
// OpExecute — the SAME E3b channel the deploy substrate's preresolve rides, so
// the provider code is placement-invisible (compiled-in vs out-of-process). The
// recorder is THIS plugin's OWN binary in recorder mode (CHARLY_ADB_RECORDER=1;
// cmd/serve) — the provider itself never dials for a session.
...
// runSession dispatches the session method against the runner service. addr/serial
// are the PROVIDER-resolved device endpoint (the check verb's container-inspect
// resolution); the recorder re-dials detached. venueDefault is the CheckEnv
// snapshot's venue id, used when the authored input carries none (evidence-row
// provenance).
func runSession(ctx context.Context, cc kit.CheckContext, env *adbEnv, in *params.AdbInput, venueDefault string) (string, error) {
	// session identity: the runner injects session_id for instruments; a PLAN-STEP
	// session (authoring adb: {method: session, action: start, session_id: x} directly
	// in a plan) falls back to "default".
	if in.SessionId == "" {
		in.SessionId = "default"
	}
	// state_dir is runner-injected for instruments; a plan-step session derives its
	// own under the host temp dir (the row.json + mp4 live there; instrument
...
// recorder process: THIS plugin's binary in recorder mode with the resolved
// device endpoint + session identity threaded via the CHARLY_ADB_* env
// (record.go). Extracted from sessionStart so the wire request is unit-testable
// without the reverse leg.
func buildSessionSpawn(in *params.AdbInput, addr, serial, exe, venue, logDir string) sessionRequest {
	if serial == "" {
		serial = "emulator-5554" // the goadb device default (env.serial()/adbDeviceForAddr)
	}
...
// sessionStart resolves nothing more (the device endpoint came from the env) — it
// submits the recorder spawn to the runner service and reports the session as
// started. state_dir is REQUIRED: the recorder (and the runner's handle) land the
// capture + evidence there, and the instrument lifecycle reads row.json back from it.
func sessionStart(ctx context.Context, cc kit.CheckContext, env *adbEnv, in *params.AdbInput, venueDefault string) (string, error) {
	if in.StateDir == "" {
		return "", fmt.Errorf("session start: state_dir required")
	}
	addr, err := env.resolveAdbAddr()
	if err != nil {
		return "", fmt.Errorf("session start: resolve device endpoint: %w", err)
	}
	...
	// log_dir: the runner's session handle + the recorder state live in the check
	// run dir. The instrument lifecycle threads the run dir over the wire
	// (buildInstrumentOp's log_dir); a plan-step session (no runner envelope) falls
	// back to "" — the runner's cwd-relative default.
	logDir := in.LogDir
	req := buildSessionSpawn(in, addr, env.serial(), exe, venue, logDir)
	if err := submitSession(ctx, cc, req); err != nil {
		return "", fmt.Errorf("session start: %w", err)
	}
	return fmt.Sprintf("session %s started (screenrecord on device %s)", in.SessionId, addr), nil
}
...
func sessionStop(ctx context.Context, cc kit.CheckContext, in *params.AdbInput) (string, error) {
	if in.StateDir == "" {
	...
	// recorder's finalize is its OWN SIGTERM trap + the device round-trips (SIGINT,
	// finalize wait, MP4 pull): the evidence row lands when the recorder writes it.
	// Wait (bounded) for the row instead of reading once.
	...
	if err := json.Unmarshal(raw, &row); err != nil {
		return "", fmt.Errorf("session stop: decode %s: %w", rowPath, err)
	}
	return fmt.Sprintf("session %s stopped: %s", in.SessionId, rowArtifact(row)), nil
}

// sessionStopRowTimeout bounds the stop's row wait. The recorder's finalize chain
// is its OWN SIGTERM trap, and the runner's stop return can beat the device
// round-trips (SIGINT → moov finalize → pull) that precede the row write. 20s is
// generous for the device chain and short enough that a crashed recorder (no row,
// ever) fails the stop fast.
const sessionStopRowTimeout = 20 * time.Second
...
// rowArtifact renders the row's first artifact as "path[kind]" (a successful stop
// path carries the <session>.mp4 artifact the recorder wrote; a failed stop shows
// "<none>").
func rowArtifact(row evidenceRow) string {
	if len(row.Artifact) == 0 {
		return "<no artifact: capture failed>"
	}
	a := row.Artifact[0]
...
func sessionStatus(ctx context.Context, cc kit.CheckContext, in *params.AdbInput) (string, error) {
